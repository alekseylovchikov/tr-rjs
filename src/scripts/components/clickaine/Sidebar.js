...
export default class Sidebar extends Component {
  static propTypes = {
    type: PropTypes.oneOf(['advertiser', 'publisher']).isRequired,
    activeState: PropTypes.oneOf(activePages).isRequired,
    ...
    const renderOneOfType = () => {
      if (type === 'publisher') {
        return PublishersSidebarItems.map(item => {
          return <SidebarItem activeClass={item.title === activePage ? true : false} type={type} switchToCurrentPage={this.handleSwitchPage.bind(this)} title={item.title} key={item.id} icon={item.icon} notification={item.notification ? item.notification : null} />;
        });
